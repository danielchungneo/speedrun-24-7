...
import Calendar from '@/components/Inputs/Base/Calendar';
...
const CTBookingScreen = () => {
  const { article } = useDemo();
    ...
    >
      ...
      <Block row marginBottom={sizes.m}>
        ...
        <Button
          ...
          onPress={() =>
            navigation.navigate(screens.CT_CHAT, { userId: option?.user?.id })
          }
        >
      ...
      <Modal visible={Boolean(modal)} onClose={() => setModal(undefined)}>
        {modal !== 'calendar' && (
          <FlatList
...
export default CTBookingScreen;
